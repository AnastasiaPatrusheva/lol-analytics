...
def render(source: str) -> None:
    ...
    # Подпись описывает график, который получился: в двадцатке лучших обычно все
    # выше половины, и отметка +0% стоит у левого края, а не посередине.
    if (top20["dev"] >= 0).all():
        direction = ("В двадцатке лучших все выше половины, поэтому столбцы растут "
                     "вправо от левого края.")
    else:
        direction = "Столбец вправо от отметки — чемпион выигрывает чаще половины, влево — реже."
    label = ("Число у столбца — его доля побед целиком." if order_col == "winrate" else
             "Число у столбца — осторожная оценка доли побед, по ней отсортирован рейтинг.")
    st.caption(
        "Отсчёт идёт от половины побед, это отметка +0%: столько выигрывает чемпион, "
        f"который не сильнее и не слабее прочих. {direction} {label}"
    )
    H = 560
    ...
    half = (
        ...
        .mark_rule(color="#a49b86", strokeWidth=1.5)
        .encode(x=alt.X("z:Q"))
    )
