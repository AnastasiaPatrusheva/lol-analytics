...
Все настройки собраны в одном месте: пути к данным, параметры Riot API,
пороги анализа, список таблиц звёздной схемы. Скрипты импортируют значения
отсюда, а не дублируют константы у себя.
"""
...
# Ремейки: при выходе игрока в первые 3 минуты матч отменяется и попадает в
# историю с длительностью 1–3 минуты. Такие матчи не отражают игру и портят
# любую статистику по длительности, поэтому отсекаем их с запасом.
MIN_MATCH_MINUTES = 5
# Длина хеша PUUID в витринах (hex). PUUID — постоянный идентификатор аккаунта
# Riot, публиковать его нельзя; для джойнов достаточно устойчивого хеша.
PLAYER_KEY_LEN = 16
