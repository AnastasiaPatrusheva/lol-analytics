...
def normalize_kaggle() -> pd.DataFrame:
    ...
    # Roles can legitimately be missing (leavers, edge cases). We
    # label them UNDEFINED instead of dropping the rows, so player
...
def main() -> None:
    ...
    # Большой источник riot_full (raw.zip) подключаем, если он уже
