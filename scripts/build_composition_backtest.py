...
Вкладка «Состав» оценивает пятёрку так: у каждого чемпиона берётся его winrate на
своей роли (осторожная оценка — нижняя граница Уилсона), пять значений усредняются.
Прогноз до сих пор ни с чем не сверялся, хотя в данных лежат десятки тысяч реальных
пятёрок с известным исходом.
...
2. Оцениваются обе команды каждого матча, и побеждает та, чья оценка выше.
   ...
   победитель, поэтому базовый уровень строго 50%, и его нельзя обыграть угадыванием.
3. Считаются две версии оценки: как в дашборде (веса — число игр чемпиона) и
   равновзвешенная (каждая роль по 1/5). Веса по числу игр отражают надёжность
   оценки, а не важность роли, поэтому сравнение показывает, не мешают ли они.
...
def _team_scores(con: duckdb.DuckDBPyConnection, source: str, patch: str) -> pd.DataFrame:
    ...
    return con.execute(f"""
        ...
        SELECT t.match_id, t.team_id,
               ...
               -- веса как в дашборде: число игр чемпиона
               SUM(COALESCE(tr.score, 0.5) * COALESCE(tr.games, 1))
                   / SUM(COALESCE(tr.games, 1)) AS score_weighted,
               -- точечная оценка winrate вместо осторожной нижней границы
