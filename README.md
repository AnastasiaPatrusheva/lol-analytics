...
Проект дата-инженерии и аналитики: полный путь данных от сбора через **Riot API**
до интерактивного дашборда на **Streamlit**.

> **Демо:** _ссылку на streamlit.app вставить после деплоя_
...
- **Extract** — `scripts/riot_data_collector.py`: инкрементальный сбор матчей (retry, rate-limit, дедуп через лог).
...
- **Игроки** — drill-down по профилю игрока.
...
- Большой объём — из Kaggle; собственная API-выборка — демонстрация ETL и свежий срез.
