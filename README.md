...
| **Предметы** — с явной оговоркой про обратную причинность | **Игроки** — архетипы KMeans по метрикам, нормированным на роль |
...
- **Три источника вместе, а не один.** Собственная свежая выборка (Riot API),
  большой архив `riot_full` (~26k матчей, 6 патчей) и Kaggle — приведены к единой
...
**1. Ни про одного чемпиона нельзя уверенно сказать, что он сильнее или слабее остальных.**
Проверка «отличается ли winrate от 50%» идёт сразу по 172 чемпионам. Без поправки на
множественные сравнения выделяются 28: 15 выигрывают чаще половины, 13 реже. Будь все
чемпионы равны, случайно выделилось бы около 9, так что в целом отличий больше, чем даёт
случай. Но кто именно из 28 выделился не случайно, сказать нельзя: с поправкой Бонферрони
(z = 3.62 вместо 1.96) не остаётся ни одного. Лидер по осторожной оценке, TwistedFate,
выигрывает 53.9% на 1691 игре: обычный интервал (51.6–56.3%) выше 50%, а интервал с
поправкой (49.5–58.3%) уже накрывает 50%. Лишние отличия могут объясняться не только силой
чемпиона, но и тем, кто на нём играет: матчи одного игрока не независимы друг от друга.
...
**3. Длина матча заметно меняет winrate лишь у немногих чемпионов.**
Разница между короткими (до 25 минут) и длинными (от 32 минут) матчами проходит проверку
с поправкой Бонферрони у 2 чемпионов из 172. Kayle выигрывает 41.6% коротких матчей и 68.3%
длинных (разрыв 26.7 пункта), у Ornn разрыв 15.8 пункта. У типичного чемпиона разрыв около
3.5 пункта: это меньше, чем 12.3 пункта между лучшим и худшим чемпионом в рейтинге. Важная
оговорка: длительность это не фактор, а следствие. Разгромные матчи заканчиваются быстро,
поэтому поражения «скейлеров» механически попадают в короткий бакет.
...
**6. KDA — не одна метрика, а две, и они показывают разные стили.**
KDA = (убийства + помощи) / смерти выглядит единым числом, но после нормировки на роль
его части почти не связаны между собой: корреляция «убийств на смерть» и «помощи на
смерть» всего 0.13, а сам KDA определяется в основном помощью (0.86 против 0.54).
Поэтому кластеризация идёт по двум отношениям вместо одного, и стили перестают сливаться:
«Керри» (1052 игрока, убийств на смерть ×1.22) отделился от «Играет на обзор» (576,
помощи на смерть ×1.44 при обзоре ×1.47). Разрыв в победах — 53.6% у «Керри» против 46.7%
у «Часто умирает» — стилем не объяснить: смерти стоят в знаменателе обоих отношений,
а в победах игроки умирают реже (4.3 смерти за матч против 6.8) и больше зарабатывают
(461 золота в минуту против 389). Плюс группы размыты (силуэт около 0.20) и смешаны
с уровнем игрока.

Оговорка ко всему разделу: рангов игроков в `riot_full` нет, поэтому неизвестно,
насколько выводы переносятся на среднего игрока.
...
- **Историчности нет.** `dim_player` схлопывает 6 патчей в один снимок: ник берётся
...
- **SQL собирается строками.** Значения приходят из фиксированных виджетов и белого
  ...
  честнее.
- **Ранги известны только у своей выборки.** `riot_api` собран с игроков Challenger,
  Grandmaster и Master. В `riot_full` и Kaggle рангов нет, поэтому неизвестно, насколько
  выводы переносятся на среднего игрока.
